 ...
 *       (iswin is set to 1) or when the board is full and there are
 *       no adjacent tiles with equal, non-0, values.
 * --------------------------------------------------------------
 */
 ...
 * void _do_replay_end():
 *
 * Update the specified game-state object (gs) so it reflects the last
 * recorded move in the replay-stack of the specified moves-history-object
 * (mvhist), and redraw the specified text-user-interface (tui).
 *
 * The last node of the replay-stack (last recorded move) gets back
 * to the caller via the (it) pointer, which then may be used as an
 * iterator.
 *
 * NOTE: The top of the replay-stack is expected to hold the oldest
 *       recorded move.
 * --------------------------------------------------------------
 */
 ...
 * void _do_replay_beg():
 *
 * Similar to the function: _do_replay_end(), but this one deals
 * with the first recorded move (instead of the last one).
 *
 * NOTE: The top of the replay-stack is expected to hold the
 *       oldest recorded move.
 * --------------------------------------------------------------
 */
 ...
 *
 * Update the specified game-state object (gs) so it reflects the next
 * recorded move in the replay stack of the specified moves-history-object
 * (mvhist), and redraw the specified text-user-interface (tui).
 *
 * The iterator pointer (it) is expected to already refer to the currently
 * viewed move in the replay-stack. It gets updated and returned back to
 * the caller as a reference to the next recorded move (if any). 
 *
 * NOTE: The top of the replay-stack is expected to hold the oldest
 *       recorded move.
 * --------------------------------------------------------------
 ...
 *
 * Similar to the function: _do_replay_next(), but this one deals
 * with the previous recorded move (instead of the next one).
 *
 * NOTE: The top of the replay-stack is expected to hold the
 *       oldest recorded move.
 * --------------------------------------------------------------
 ...
 *
 * Auto-play all recorded moves available in the replay-stack of the
 * specified moves-history object (mvhist), starting from the currently
 * viewed move, referred to by the specified iterator pointer (it).
 *
 * In each iteration of the auto-play loop, the specified game-state
 * object (gs) gets updated accordingly, and the specified text-user-
 * interface object (tui) is redrawn.
 *
 * When auto-play finishes, the updated iterator pointer gets back
 * to the caller as a reference to the last recorded move.
 *
 * NOTE: The top of the replay-stack is expected to hold the oldest
 *       recorded move.
 * --------------------------------------------------------------
 ...
 *
 * Save to disk the currently viewed replay, along with all other
 * needed info available in the specified moves-history object (mvhist)
 * and redraw the specified text-user-interface object (tui).
 * --------------------------------------------------------------
 */
 ...
 *
 * Use the specified text-user-interface object (tui) to ask for
 * a replay-file, and load it. All the arguments of the function
 * get updated according to the contents of loaded replay-file.
 *
 * NOTES (IMPORTANT!):
 *
 *     When a replay-file gets loaded successfully, it overwrites
 *     completely the previous game-state (gs). In case the size
 *     of the loaded board was different from the size of the
 *     overwritten one, realloc() may have moved the board to
 *     a different memory location.
 *
 *     So, it is really IMPORTANT that the text-user-interface gets
 *     well aware of this possible change, by calling the function:
 *     tui_update_board_reference()
 *
 *     On a similar note, in an effort to (arguably) simplify the code
 *     a bit, when loading a replay-file, instead of updating directly
 *     the specified moves-history object (mvhist), first I free it,
 *     then I load the file into a fresh, temporary moves-history object
 *     (tmp) and finally I tell the pointer of the original object to
 *     point to the newly created one.
 *
 *     This, once again, most probably moves the original mvhist object
 *     to different memory location, and the text-user-interface MUST
 *     know about it. So, it is really IMPORTANT to also call the func:
 *     tui_update_mvhist_reference()
 * --------------------------------------------------------------
 */
 ...
 * (won or not). 1 (true) is also returned in case of an error,
 * since it makes sense for the caller to be notified with a game-over
