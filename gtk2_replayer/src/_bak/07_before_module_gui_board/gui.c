...
static inline void _remove_board_tiles( Gui *gui, int dim )
{
	...
	for (i=0; i < dim; i++) {
		for (j=0; j < dim; j++) {
			...
			memset(&gui->board.tiles[n].ebname,  0, SZMAX_DBGMSG);
			memset(&gui->board.tiles[n].lblname, 0, SZMAX_DBGMSG);
		}
	}
}
 ...
 * ---------------------------------------------------
 */
static inline gboolean _create_board_tiles(
	Gui     *gui,
	int      dim,
	long int imove
	)
...
//gamedata_print_tiles( gui->gamedata );
...
int tileval = 0;
if ( -1 == imove
|| 0 == (tileval = gamedata_get_tile_of_move(gui->gamedata, n, imove))
){
	g_snprintf(
		tilemarkup,
		SZMAX_DBGMSG,
...
//	gtk_table_resize( GTK_TABLE(gui->board.table), DIM, DIM );
	...
	/* board tiles */
	if ( !_create_board_tiles(gui, dim, -1) ) {
		DBG_GUI_ERRMSG(
			gui->appWindow,
 ...
 * ---------------------------------------------------
 */
gboolean gui_resize_board( Gui *gui, long int imove )
{
	guint curdim = 0;
	int   dim = 0;
	...
	gtk_table_resize( GTK_TABLE(gui->board.table), dim, dim );
	if ( !_create_board_tiles(gui, dim, imove) ) {
		DBG_GUI_ERRMSG(
			gui->appWindow,
			"_create_board_tiles() failed!"
			);
