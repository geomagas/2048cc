 ...
 * ---------------------------------------------------
 */
static inline void _prepare_tile_style(
	const GuiBoard *board,
	long int       imove,
	int            dim,
	int            itile,
	const Gui      *gui,
	char tilemarkup[SZMAX_DBGMSG],  /* out */
	const _TileColors **tilecolors  /* out */
	)
{
	long int tileval = 0;
	if ( imove > -1 ) {
	...
	(*tilecolors) = _tileval_to_tilecolors( tileval, &board->colors );
}
...
#else
long int tileval = 0;
long int imove   = gui_get_imove( gui );
if ( imove > -1 ) {
	tileval = gui_get_gamedata_tile_of_move(
			gui,
			n,
...
tilecolors = _tileval_to_tilecolors( tileval, &gb->colors );
g_snprintf(
	tilemarkup,
	SZMAX_DBGMSG,
	"<span font=\"%d\" font_weight=\"bold\">%ld</span>",
 ...
 * ---------------------------------------------------
 */
gboolean gui_board_resize_from_gamedata( GuiBoard *gb, Gui *gui )
{
	guint curdim = 0;
	int   dim = 0;
