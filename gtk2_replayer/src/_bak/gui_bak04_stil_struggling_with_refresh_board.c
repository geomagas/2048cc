 ...
 * ---------------------------------------------------
 */
static inline gboolen _create_board_tiles( Gui *gui, int dim )
{
	int i,j;
...
//	gtk_widget_destroy( gui->board.ebContainer );
	gtk_widget_destroy( gui->board.table );
//	gtk_container_remove(
//		GTK_CONTAINER( gui->board.ebContainer ),
//		gui->board.table
//		);
	gui->board.table = gtk_table_new( 7, 7, TRUE );
	gtk_container_add(
		GTK_CONTAINER( gui->board.ebContainer ),
		gui->board.table
		);
...
#else
//	gtk_widget_destroy( gui->board.table );
//	gui->board.table = gtk_table_new( 4, 4, TRUE );
//	gtk_container_add(
//		GTK_CONTAINER( gui->board.ebContainer ),
//		gui->board.table
//		);
	...
	const int dim = gamedata_get_dim( gui->gamedata );
	if ( dim < 4 ) {
		DBG_GUI_ERRMSG(
			gui->appWindow,
			"gui->gamedata.dim < 4!"
	...
	/* draw tiles of the resized board */
	for (i=0; i < dim; i++) {
		for (j=0; j < dim; j++) {
...
//			gtk_widget_modify_bg(button, GTK_STATE_ACTIVE, &blue);
			...
			gtk_widget_set_name( tile.evbox, tile.ebname );
