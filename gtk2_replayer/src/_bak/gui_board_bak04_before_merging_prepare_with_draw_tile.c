 ...
 * ---------------------------------------------------
 */
static inline void _prepare_tile_visuals(
	const _BoardSkin  *skin,
	long int          imove,
	int               dim,
	int               itile,
	...
	)
{
	...
	(*tilecolors) = _tileval_to_tilecolors( tileval, skin );
}
...
//dbg_gamedata_print_tiles( gui_get_gamedata(gui) );
	...
	for (i=0; i < dim; i++) {
		for (j=0; j < dim; j++) {
			...
			_prepare_tile_visuals(
				&gb->skin,
				gui_get_imove( gui ),
				dim,
				n,
				gui,
				tilemarkup,  /* by ref - gets updated */
				&tilecolors  /* by ref - gets updated */
				);
