...
if ( fname ) {
	gui_set_gamedata_from_fname( gui, fname );
	gui_refresh_statusbar_txtout( gui );
#if 0
	char *fn = s_new_shortfname( fname );
	gchar txt[MAXSIZ_DBGMSG] = {'\0'};
	g_snprintf(
		txt,
		MAXSIZ_DBGMSG,
		TXTF_STATUSBAR,
	...
	gtk_statusbar_push(
		GTK_STATUSBAR( gui_get_statusbar_widget(gui) ),
		gui_get_statusbar_contextId(gui),
		txt
		);
	free( fn );
#endif
	gtk_widget_set_sensitive( gui_get_statusbar_widget(gui), TRUE );
}
		// open_file( fname );
		g_free( fname );
	}
