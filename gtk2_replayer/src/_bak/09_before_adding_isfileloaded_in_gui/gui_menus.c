...
//				gui_set_imove( gui, gui_get_gamedata_nmoves(gui)-1 );
				gui_refresh_scores( gui );
				gui_resize_and_refresh_board( gui );
				gui_refresh_prevnext( gui );
				gui_refresh_statusbar_txtout( gui );
...
#if 1
	gui_menus_refresh( gui_get_menus(gui), gui );
#else
	GuiMenus *menus = gui_get_menus(gui);
	gtk_widget_set_sensitive( menus->playback.miFirst, TRUE );
	gtk_widget_set_sensitive( menus->playback.miPrevious, TRUE );
#endif
	return;
}
