 ...
 */
struct _Gui {
	gint wmax, hmax;  /* width & height of user's screen */
 ...
 * ---------------------------------------------------
 */
void on_clicked_toolResetspeed( GtkWidget *tool, Gui *gui )
{
	/* avoid compiler warning for unused parameter */
	(void)tool;
	...
	GtkWidget *mireset = gui_menus_get_mirPlaybackSpeedReset(
				gui_get_menus( gui )
				);
//	mygtk_alert_box( gui_get_appWindow(gui), "Tool: Reset Speed" );
//	g_signal_emit_by_name( mireset, "activate" );
	gtk_menu_item_activate( GTK_MENU_ITEM(mireset) );
 ...
 * ---------------------------------------------------
 */
static inline gboolean _init_toolbar_playback( Gui *gui, GtkBuilder *builder )
{
	/* just for brevity later on */
...
*/
	...
	*pw = GTK_WIDGET(
	...
	g_signal_connect(
		G_OBJECT( *pw ),
		"clicked",
		G_CALLBACK( on_clicked_toolResetspeed ),
...
static inline gboolean _init_from_glade_file( Gui *gui, const gchar *fnameGlade )
{
	...
	_init_scores( gui, builder );
	_init_board( gui, builder );
	_init_toolbar_playback( gui, builder );
