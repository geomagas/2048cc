 ...
 * ---------------------------------------------------
 */
static inline gboolean _read_os( GuiLocale *gl )
{
	gchar *locale      = NULL;
	gchar *varLang     = NULL;
	gchar *varLanguage = NULL;

	/* get current locale environment from os */
	locale      = setlocale( LC_ALL, NULL );
	varLang     = (gchar *) g_getenv("LANG");
	varLanguage = (gchar *) g_getenv("LANGUAGE");

	if ( !locale || !varLang || !varLanguage ) {
		return FALSE;
	}

	strncpy( gl->os.locale, locale, MAXSIZ_LOCALE-1 );
	gl->os.varLang      = varLang;
	gui->os.varLanguage = varLanguage;
 ...
 * ---------------------------------------------------
 */
static inline void _restore_os( GuiLocale *gl )
{
	setlocale(LC_ALL, gl->os.locale);
 ...
 * ---------------------------------------------------
 */
static inline gboolean _apply_env( GuiLocale *gl )
{
	if ( NULL == setlocale(LC_ALL, gl->env.locale) {
	...
	if ( !g_setenv( "LANG", gl->env.varLang, TRUE ) ) {
	...
	if ( !g_setenv( "LANGUAGE", gl->env.varLanguage, TRUE ) {
 ...
 * ---------------------------------------------------
 */
gboolean gui_locale_apply_en( GuiLocale *gl )
{
	if ( NULL == gl ) {
	...
	gl->env.locale      = (gchar *) STR_EN_LOCALE;
	gl->env.varLang     = (gchar *) STR_EN_LOCALE_LANG;
	gl->env.varLanguage = (gchar *) STR_EN_LOCALE_LANGUAGE;

	return _apply_env( gl );
}
 ...
 * ---------------------------------------------------
 */
gboolean gui_locale_apply_el( GuiLocale *gl )
{
	if ( NULL == gl ) {
		DBG_GUI_ERRMSG( NULL, "NULL pointer argument!");
		return FALSE;
	...
	gl->env.locale      = (gchar *) STR_EL_LOCALE;
	gl->env.varLang     = (gchar *) STR_EL_LOCALE_LANG;
	gl->env.varLanguage = (gchar *) STR_EL_LOCALE_LANGUAGE;
 ...
 * ---------------------------------------------------
 */
gboolean gui_locale_apply_osenv( GuiLocale *gl )
{
	if ( NULL == gl ) {
		DBG_GUI_ERRMSG( NULL, "NULL pointer argument!");
		return FALSE;
...
GuiLocale *new_gui_locale( void )
{
	...
	/* Initialize internal locale to current locale environment from os,
	 * or default to English if no proper os locale was found.
	 */
