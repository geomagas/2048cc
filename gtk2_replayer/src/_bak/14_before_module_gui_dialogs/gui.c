 ...
 * ---------------------------------------------------
 */
static inline gboolean _init_dlg_jumpto_from_builder(
	Gui         *gui,
	GtkBuilder  *builder
	)
{
//	char txtout[SZMAX_DBGMSG] = {'\0'};

	/* load the jumpto-dialog widgets into gui, from the GTK+2 builder */

	gui->dlgJumpto.root
	= GTK_WIDGET( gtk_builder_get_object( builder, "dlgJumpto") );

	gui->dlgJumpto.lblRange
	= GTK_WIDGET( gtk_builder_get_object( builder, "lblJumptoRange") );

	gui->dlgJumpto.lblCurrent
	= GTK_WIDGET( gtk_builder_get_object( builder, "lblJumptoCurrent") );

	gui->dlgJumpto.te
	= GTK_WIDGET( gtk_builder_get_object( builder, "teJumpto") );

	/* set dialog's initial position */
	gtk_window_set_position(
		GTK_WINDOW( gui->dlgJumpto.root ),
		GTK_WIN_POS_CENTER
		);
	...
	/* set dialog's opacity */
	gtk_window_set_opacity( GTK_WINDOW(gui->dlgJumpto.root), 0.90 );

	/* */
	g_signal_connect(
		G_OBJECT( gui->dlgJumpto.root ),
		"destroy",
		G_CALLBACK( gtk_widget_destroyed ),
		&gui->dlgJumpto.root
		);
...
static inline gboolean _init_from_glade_file( Gui *gui, const gchar *fnameGlade )
{
	...
	_init_dlg_jumpto_from_builder( gui, builder );
 ...
 * ---------------------------------------------------
 */
GtkWidget *gui_get_dlgJumpto_root( Gui *gui )
{
	if ( NULL == gui ) {
		DBG_STDERR_MSG( "NULL pointer argument (gui)!" );
		return NULL;
 ...
 * ---------------------------------------------------
 */
GtkWidget *gui_get_dlgJumpto_lblRange( Gui *gui )
{
	if ( NULL == gui ) {
		DBG_STDERR_MSG( "NULL pointer argument (gui)!" );
		return NULL;
 ...
 * ---------------------------------------------------
 */
GtkWidget *gui_get_dlgJumpto_lblCurrent( Gui *gui )
{
	if ( NULL == gui ) {
		DBG_STDERR_MSG( "NULL pointer argument (gui)!" );
		return NULL;
	...
	return gui->dlgJumpto.lblCurrent;
}
 ...
 * ---------------------------------------------------
 */
GtkWidget *gui_get_dlgJumpto_te( Gui *gui )
{
	if ( NULL == gui ) {
		DBG_STDERR_MSG( "NULL pointer argument (gui)!" );
		return NULL;
