...
void on_activate_miPlaybackJumpto( GtkWidget *mi, Gui *gui )
{
	...
	GtkWidget *dlgJumptoRoot = NULL;
	char *txtJumpto = NULL;         /* for copying text from widget */
	char *tailptr = NULL;           /* for strtol() */
	long int jumpto = 0;            /* widget's text as long int */
	gchar txt[SZMAX_DBGMSG] = {'\0'};
	long int rangeFirst = -1, rangeLast = -1;
	GtkEntry *te = NULL;
...
//	mygtk_alert_box( gui_get_appWindow(gui), "Playback -> Jump to" );
	...
	gtk_widget_set_sensitive( gui_get_appWindow(gui), FALSE );

	te         = GTK_ENTRY( gui_get_dlgJumpto_te(gui) );
	rangeFirst = 1L;
	rangeLast  = gui_get_gamedata_nmoves( gui );

	/* set text of label-widget: range */
	g_snprintf( txt, SZMAX_DBGMSG,
		TXTF_DLG_JUMPTO_RANGE,
		rangeFirst,
		rangeLast
		);
	gtk_label_set_text(
		GTK_LABEL( gui_get_dlgJumpto_lblRange(gui) ),
		txt
		);
	...
	gtk_label_set_text(
		GTK_LABEL( gui_get_dlgJumpto_lblCurrent(gui) ),
		txt
		);
	...
	dlgJumptoRoot = gui_get_dlgJumpto_root( gui );
	response = gtk_dialog_run( GTK_DIALOG(dlgJumptoRoot) );

	if ( GUI_RESPONSE_OK == response ) {
		errno = 0;
		txtJumpto = s_dup( gtk_entry_get_text(te) );
		jumpto    = strtol( txtJumpto, &tailptr, 10 );
		if ( ERANGE != errno
		&& '\0' == *tailptr
		&& jumpto >= rangeFirst
		&& jumpto <= rangeLast
	...
	gtk_widget_hide( GTK_WIDGET(dlgJumptoRoot) );

	gtk_widget_set_sensitive( gui_get_appWindow(gui), TRUE );
...
gboolean gui_menus_refresh( GuiMenus *gm, const Gui *gui )
{
	...
	/* when replay-file is loaded */
		...
		/* Speed->mirFaster */
		if ( gui_get_delay(gui) > gui_get_stepDelay(gui) ) {
			gtk_widget_set_sensitive(
