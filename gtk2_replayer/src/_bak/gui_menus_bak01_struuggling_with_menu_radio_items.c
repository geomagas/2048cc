 ...
 * ---------------------------------------------------
 */
static gboolean _enable_playback_speed_mi_as_radio(
	GtkWidget *mi,
	Gui       *gui
	)
{
	/* temp ptrs for brevity typing later on */
	GuiMenus  *guimenus = gui_get_menus( gui );
	if ( NULL == guimenus || NULL == mi ) {
		DBG_GUI_ERRMSG(
	...
	GtkWidget *reset  = guimenus->playback.speed.mirReset;
	GtkWidget *slower = guimenus->playback.speed.mirSlower;
	GtkWidget *faster = guimenus->playback.speed.mirFaster;
	if ( NULL == reset ) {
		DBG_GUI_ERRMSG(
			gui_get_appWindow( gui ),
			"Invalid GUI element (menus->speed.mirReset)"
			);
		return FALSE;
	}
	if ( NULL == slower ) {
		DBG_GUI_ERRMSG(
			gui_get_appWindow( gui ),
			"Invalid GUI element (menus->speed.mirSlower)"
			);
		return FALSE;
	}
	if ( NULL == faster ) {
		DBG_GUI_ERRMSG(
			gui_get_appWindow( gui ),
			"Invalid GUI element (menus->speed.mirFaster)"
			);
		return FALSE;
	}

	/* first disable all items */
//	gtk_check_menu_item_set_active( GTK_CHECK_MENU_ITEM(reset),  FALSE );
//	gtk_check_menu_item_set_active( GTK_CHECK_MENU_ITEM(slower), FALSE );
//	gtk_check_menu_item_set_active( GTK_CHECK_MENU_ITEM(faster), FALSE );

	/* then enable only the requested item (mi) */
//	gtk_check_menu_item_set_active( GTK_CHECK_MENU_ITEM( mi ), TRUE );
/*
	if ( mi == reset ) {
		gtk_check_menu_item_set_active(
			GTK_CHECK_MENU_ITEM( reset ),
			TRUE
			);
	}
	else if ( mi == slower ) {
		gtk_check_menu_item_set_active(
			GTK_CHECK_MENU_ITEM( slower ),
			TRUE
			);
	}
	else if ( mi == osmi ) {
		gtk_check_menu_item_set_active(
			GTK_CHECK_MENU_ITEM( faster ),
			TRUE
			);
	}
*/
...
/* ---------------------------------------------------
 * Callback function connected to the GTK+ "activate"
 * signal, for the menu-item "Playback->Speed->Reset".
 *
 * NOTE: Public, because also used by the playback-toolbar
 * ---------------------------------------------------
 */
void on_groupchanged_menuradiogroup_PlaybackSpeed( GtkWidget *mi, Gui *gui )
{
	/* avoid compiler warning for unused parameter */
	if ( NULL == mi ) {
		return;
	...
	GuiMenus *menus = gui_get_menus( gui );

	if ( gtk_check_menu_item_get_active(
		GTK_CHECK_MENU_ITEM( menus->playback.speed.mirReset )
		)
	){
		mygtk_alert_box(
			gui_get_appWindow( gui ),
		...
		GTK_CHECK_MENU_ITEM( menus->playback.speed.mirSlower )
		)
	){
		mygtk_alert_box(
			gui_get_appWindow( gui ),
		...
		GTK_CHECK_MENU_ITEM( menus->playback.speed.mirFaster )
		)
	){
		mygtk_alert_box(
			gui_get_appWindow( gui ),
 ...
 * ---------------------------------------------------
 */
void on_activate_mirPlaybackSpeedReset( GtkWidget *mi, Gui *gui )
{
	/* avoid compiler warning for unused parameter */
	if ( NULL == mi ) {
		return;
...
//	_enable_playback_speed_mi_as_radio( mi, gui );
...
/* ---------------------------------------------------
 * Callback function connected to the GTK+ "activate"
 * signal, for the menu-item "Playback->Speed->Slower".
 *
 * NOTE: Public, because also used by the playback-toolbar
 * ---------------------------------------------------
 */
void on_activate_mirPlaybackSpeedSlower( GtkWidget *mi, Gui *gui )
{
	/* avoid compiler warning for unused parameter */
	if ( NULL == mi ) {
		return;
...
//	_enable_playback_speed_mi_as_radio( mi, gui );
	mygtk_alert_box(gui_get_appWindow(gui), "Playback -> Speed -> Slower");
...
/* ---------------------------------------------------
 * Callback function connected to the GTK+ "activate"
 * signal, for the menu-item "Playback->Speed->Faster".
 *
 * NOTE: Public, because also used by the playback-toolbar
 * ---------------------------------------------------
 */
void on_activate_mirPlaybackSpeedFaster( GtkWidget *mi, Gui *gui )
{
	/* avoid compiler warning for unused parameter */
	if ( NULL == mi ) {
		return;
...
//	_enable_playback_speed_mi_as_radio( mi, gui );
	mygtk_alert_box(gui_get_appWindow(gui), "Playback -> Speed -> Faster");
	...
	)
{
	...
	*pw = GTK_WIDGET(
	...
	g_signal_connect(
		...
		"activate",
//		"group-changed",
		G_CALLBACK( on_groupchanged_menuradiogroup_PlaybackSpeed ),
//		G_CALLBACK( on_activate_mirPlaybackSpeedReset ),
		gui
		);
	g_signal_connect(
		G_OBJECT( *pw ),
...
//		"group-changed",
		G_CALLBACK( on_groupchanged_menuradiogroup_PlaybackSpeed ),
//		G_CALLBACK( on_activate_mirPlaybackSpeedSlower ),
		gui
		);
	g_signal_connect(
		G_OBJECT( *pw ),
...
//		"group-changed",
		G_CALLBACK( on_groupchanged_menuradiogroup_PlaybackSpeed ),
//		G_CALLBACK( on_activate_mirPlaybackSpeedFaster ),
		gui
		);
	g_signal_connect(
		G_OBJECT( *pw ),
