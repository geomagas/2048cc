...
**Latest version**: 0.2a (August 6, 2014)  
...
and Linux (and possibly on other platforms too). and for giving you
some hints in order to get around the source code in case you decide to browse
and/or modify it.
...
your changes will take priority over the previous contents, put then before the
old contents).
...
that in the past it was a real pain. However, according the
...
like putting some serious effort to explain how to build a GTK+ development
...
I also advice against building 64-bit GTK+ applications.**
  ...
  >the the Windows' native command-prompt.
  >
  >Among lots of other things, it can interpret backticks, which as I explain
  >in the following section (*"Compiling GTK+2 Applications"*) can really help
  >us simplifying the compilation of any GTK+2 application.
...
system, as described in the previous section (*"Installing the GTK+2 Development
Environment"*), you can open a terminal or a command-prompt window and type the
following 2 lines:
...
of any C tool-chain that understands the `-I` and `-L` command-line
...
GTK+2 *hello.c* source file using `gcc` and the `pkg-config` utility, is heavily
simplified into the following line:

    cc hello.c `pkg-config --cflags --libs gtk+-2.0`

As described in the section *"Installing a GCC/MinGW-GCC Tool Chain"*, on
Windows you need a replacement of the native command-prompt, capable of
