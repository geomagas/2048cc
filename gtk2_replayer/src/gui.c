 ...
 * void gui_alert_box():
 *
 * Display the specified message inside a simple modal
 * dialog.
 *
	...
	)
{
	...
	gtk_window_set_title( GTK_WINDOW(alertBox), _(TXT_ALERT_BOX) );
...
/* ---------------------------------------------------
 * void gui_widget_redraw()
 *
 * Force a redraw of the specified gtk-widget.
 *
 ...
 * ---------------------------------------------------
 */
void gui_widget_redraw( GtkWidget *widget )
{
	/* sanity check */
	if ( !widget ) {
		return;
	...
	)
{
	...
	if ( gui && gui->isPlayPressed ) {
		gui->isPausePressed = TRUE;
		while( gtk_events_pending() ) {
			gtk_main_iteration();
		}
...
//	gtk_widget_set_size_request(gui->appWindow, 800, 600);
	gint h = (gui->hmax <= 800) ? gui->hmax - 85 : 800;
...
static inline gboolean _init_from_glade_file( Gui *gui, const gchar *fnameGlade )
{
	...
	gui_slider_playback_init_from_builder(
		gui->sliderPlayback,
		builder,
		gui
		);
...
Gui *gui_free( Gui *gui )
{
	if ( gui ) {
		...
		gui_slider_free( gui->sliderPlayback );
	...
	)
{
	...
	/* instantiate gui's playback-slider abstraction */
	gui->sliderPlayback = make_gui_slider();
	if ( NULL == gui->sliderPlayback ) {
		DBG_STDERR_MSG( "(fatal error) make_gui_slider() failed!" );
		goto ret_failure;
	}
 ...
 * ---------------------------------------------------
 */
gboolean gui_set_slider_playback_value( Gui *gui, gdouble val )
{
	if ( NULL == gui ) {
		DBG_STDERR_MSG( "NULL pointer argument (gui)!" );
		return FALSE;
	...
	return gui_slider_set_value( gui->sliderPlayback, val );
}
 ...
 * ---------------------------------------------------
 */
gboolean gui_set_slider_playback_page_increment_10per(
	Gui    *gui,
	gdouble minVal,
	gdouble maxVal
	)
{
	...
	return gui_slider_set_page_increment_10percent(
			gui->sliderPlayback,
			minVal,
			maxVal
			);
...
gboolean gui_set_delay_smaller( Gui *gui )
{
	...
	if ( gui->delay >= GUI_MIN_PLAYBACK_DELAY + gui->stepDelay ) {
		gui->delay -= gui->stepDelay;
	}
	else {
		gui->delay = GUI_MIN_PLAYBACK_DELAY;
	}
...
gboolean gui_set_delay_bigger( Gui *gui )
{
	...
	gui->delay += gui->stepDelay;
	if ( gui->delay > GUI_MAX_PLAYBACK_DELAY ) {
		gui->delay = GUI_MAX_PLAYBACK_DELAY;
	}
...
//	}
	...
	gui_menus_refresh( gui->menus, gui );
	_refresh_scores( gui );
	gui_board_refresh( gui->board, gui);
	gui_slider_playback_refresh( gui->sliderPlayback, gui );
