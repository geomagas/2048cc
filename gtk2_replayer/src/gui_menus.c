...
//				gui_set_imove( gui, gui_get_gamedata_nmoves(gui)-1 );
				...
				gui_set_delay(
					gui,
					(gulong)gui_get_gamedata_delay( gui )
					);
				gui_resize_board( gui );
				gui_set_slider_playback_value_limits(
					gui,
					1.0,
					(gdouble) gui_get_gamedata_nmoves(gui)
					);
				gui_set_slider_playback_page_increment_10per(
					gui,
					1.0,
					(gdouble) gui_get_gamedata_nmoves(gui)
					);
	...
	)
{
	...
	if ( mi == gui_menus_get_mirPlaybackSpeedReset( gui_get_menus(gui) ) ){
		gui_set_delay(
			gui,
			(gulong)gui_get_gamedata_delay(gui)
			);
		...
		mi == gui_menus_get_mirPlaybackSpeedSlower( gui_get_menus(gui))
	){
		gui_set_delay_bigger( gui );
	}
	else if (
		mi == gui_menus_get_mirPlaybackSpeedFaster( gui_get_menus(gui))
	){
		gui_set_delay_smaller( gui );
	}
...
//		);
	// gui_alert_box( gui_get_appWindow(gui), "Playback -> Speed -> Reset" );
...
//	gui_alert_box( gui_get_appWindow(gui), "Playback -> Play" );

	gui_set_isPlayPressed( gui, TRUE );
	while ( !gui_get_isPausePressed( gui )
	&& !gui_get_isStopPressed( gui )
	&& gui_has_imove_next( gui )
	){
		gui_set_imove_next( gui );
		gui_refresh( gui );
		gui_widget_redraw( gui_get_board_container(gui) );
		g_usleep( 1000 * gui_get_delay(gui) );
	}
	if ( gui_get_isStopPressed(gui) ) {
		gui_set_imove( gui, 0 );
	}
	gui_set_isPlayPressed( gui, FALSE );
	gui_set_isPausePressed( gui, FALSE );
	gui_set_isStopPressed( gui, FALSE );
	gui_refresh( gui );
...
//	gui_alert_box( gui_get_appWindow(gui), "Playback -> Pause" );

	gui_set_isPausePressed( gui, TRUE );
//	gui_set_isPlayPressed( gui, TRUE );
...
//	gui_alert_box( gui_get_appWindow(gui), "Playback -> Stop" );
	gui_set_isStopPressed( gui, TRUE );
...
//	gui_alert_box( gui_get_appWindow(gui), "Playback -> First" );
...
//	gui_alert_box( gui_get_appWindow(gui), "Playback -> Previous" );
...
//	gui_alert_box( gui_get_appWindow(gui), "Playback -> Next" );
...
//	gui_alert_box( gui_get_appWindow(gui), "Playback -> Last" );
...
//	gui_alert_box( gui_get_appWindow(gui), "Playback -> Jump to" );
	gui_do_dialog_jumpto( gui );
 ...
 * ---------------------------------------------------
 */
static void _on_activate_miHelpAbout( GtkWidget *mi, Gui *gui )
{
	GtkWidget *appwind  = NULL;
	...
	appwind  = gui_get_appWindow( gui );
	dlgabout = gui_get_dialog_about_root( gui );
	...
	)
{
	...
	*pw = GTK_WIDGET( gtk_builder_get_object(builder, "miPlaybackJumpto") );
	gtk_menu_item_set_label(
		GTK_MENU_ITEM( *pw ),
		_( TXT_MENUITEM_PLAYBACK_JUMPTO )
		);
	g_signal_connect(
		G_OBJECT( *pw ),
		"activate",
	...
	)
{
	...
	g_signal_connect(
		...
		"activate",
		G_CALLBACK( _on_activate_miHelpAbout ),
		gui
		);
	g_signal_connect(
		G_OBJECT( menus->help.miAbout ),
 ...
 * into my GUI abstraction: gui->menus (GuiMenus), it connects
...
gboolean gui_menus_refresh( GuiMenus *gm, const Gui *gui )
{
	...
	/* when replay-file is loaded */
		...
		/* miPlay */
		if ( !gui_has_imove_next(gui) || gui_get_isPlayPressed(gui) ) {
			gtk_widget_set_sensitive( gm->playback.miPlay, FALSE );
		}
		else {
			gtk_widget_set_sensitive( gm->playback.miPlay, TRUE );
		}
		...
		&& !gui_get_isStopPressed( gui )
			...
			gtk_widget_set_sensitive( gm->playback.miPause, TRUE );
			gtk_widget_set_sensitive( gm->playback.miStop, TRUE );
		}
		else {
			gtk_widget_set_sensitive( gm->playback.miPause, FALSE);
			gtk_widget_set_sensitive( gm->playback.miStop, FALSE );
		}

		/* miFirst & miPrevious */
		if ( gui_has_imove_prev(gui) && !gui_get_isPlayPressed(gui) ) {
		...
		/* Speed->mirReset */
		if ( gui_get_delay(gui) != gui_get_gamedata_delay(gui) ) {
			gtk_widget_set_sensitive(
				gm->playback.speed.mirReset,
				TRUE
				);
		}
		else {
		...
		/* Speed->mirSlower */
		if (
		gui_get_delay(gui) < GUI_MAX_PLAYBACK_DELAY ){
			gtk_widget_set_sensitive(
				gm->playback.speed.mirSlower,
				TRUE
				);
		}
		else {
		...
		gui_get_delay(gui)
		>=
		GUI_MIN_PLAYBACK_DELAY + gui_get_stepDelay(gui)
		){
			gtk_widget_set_sensitive(
				gm->playback.speed.mirFaster,
				TRUE
				);
		}
		else {
	...
	gtk_widget_set_sensitive( gm->lang.mt, FALSE );
